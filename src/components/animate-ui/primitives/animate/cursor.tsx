...
function CursorProvider({ children, global = false }: CursorProviderProps) {
  ...
  React.useEffect(() => {
    ...
    if (global) {
      ...
      parent.addEventListener('pointerout', handlePointerOut, { passive: true });
...
type CursorProps = WithAsChild<HTMLMotionProps<'div'> & { children: React.ReactNode }>;
...
function CursorFollow({
  ...
  React.useImperativeHandle(ref as React.Ref<HTMLDivElement>, () => cursorFollowRef.current as HTMLDivElement);
  ...
  const calculateOffset = React.useCallback(() => {
    ...
    switch (side) {
      case 'top':
        ...
        switch (align) {
          case 'start': offsetX = width + alignOffset; break;
          case 'center': offsetX = width / 2; break;
          case 'end': offsetX = -alignOffset; break;
        }
        break;
      case 'bottom':
        offsetY = -sideOffset;
      ...
      case 'left':
        ...
        switch (align) {
          case 'start': offsetY = height + alignOffset; break;
          case 'center': offsetY = height / 2; break;
          case 'end': offsetY = -alignOffset; break;
        }
        break;
      case 'right':
        offsetX = -sideOffset;
